...
| v7.4 | `app/` (active development — "Refine + Rearrange": v7.1's findable tools / work-that-saves / relief-admin fixes / accessibility, the Find-a-Teacher availability checker with a date picker, and a big interactive live seating chart in Present — free-drag students anywhere + move furniture, Save/Reset; v7.4 enlarges the Present seat map to fill the screen and makes "Open" on a linked layout carry the room + seated students into the designer) | `/pedagogies/` (via `index.html`) | `cocher_*` (incl. `cocher_app_data`) | **Current** |
