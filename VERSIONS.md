...
| v7.3 | `app/` (active development — "Refine + Rearrange": v7.1's findable tools / work-that-saves / relief-admin fixes / accessibility, the Find-a-Teacher availability checker with a date picker, and a big interactive live seating chart in Present — free-drag students anywhere + move furniture, Save/Reset) | `/pedagogies/` (via `index.html`) | `cocher_*` (incl. `cocher_app_data`) | **Current** |
