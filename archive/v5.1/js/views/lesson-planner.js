...
const PLANNER_REF_KEY = 'cocher_v5_1_planner_reference_ids';
...
function consumePendingSpatialLayout() {
  const pendingId = sessionStorage.getItem('cocher_v5_1_pending_spatial_layout');
  if (!pendingId || !currentLessonId) return;
  sessionStorage.removeItem('cocher_v5_1_pending_spatial_layout');
...
const LP_PREFS_KEY = 'cocher_v5_1_lp_prefs';
function getLPPrefs() {
...
const EEE_KEY = 'cocher_v5_1_eee_selections';
const EEE_SIDEBAR_KEY = 'cocher_v5_1_eee_sidebar';
...
const CUSTOM_LINKS_KEY = 'cocher_v5_1_custom_links';
...
export function render(container) {
  ...
  const planClassId = sessionStorage.getItem('cocher_v5_1_plan_class_id');
  if (planClassId && !currentLessonId && chatMessages.length === 0) {
    planClassContext = {
      id: planClassId,
      name: sessionStorage.getItem('cocher_v5_1_plan_class_name') || '',
      subject: sessionStorage.getItem('cocher_v5_1_plan_class_subject') || '',
      level: sessionStorage.getItem('cocher_v5_1_plan_class_level') || ''
    };
    sessionStorage.removeItem('cocher_v5_1_plan_class_id');
    sessionStorage.removeItem('cocher_v5_1_plan_class_name');
    sessionStorage.removeItem('cocher_v5_1_plan_class_subject');
    sessionStorage.removeItem('cocher_v5_1_plan_class_level');
  }

  // Pick up reflection insights from previous lesson
  const reflectionInsightsRaw = sessionStorage.getItem('cocher_v5_1_reflection_insights');
  ...
  if (reflectionInsightsRaw && !currentLessonId && chatMessages.length === 0) {
    try {
      ...
      sessionStorage.removeItem('cocher_v5_1_reflection_insights');
    } catch {}
  } else {
    sessionStorage.removeItem('cocher_v5_1_reflection_insights');
  }
  ...
  const incomingSpatialId = sessionStorage.getItem('cocher_v5_1_link_spatial_layout');
  if (incomingSpatialId) {
    sessionStorage.removeItem('cocher_v5_1_link_spatial_layout');
    ...
    if (!currentLessonId) {
      sessionStorage.setItem('cocher_v5_1_pending_spatial_layout', incomingSpatialId);
    }
  }
...
Maximum 3-5 recommendations. If nothing genuinely fits, say so — don't pad with irrelevant items.`,
  ...
  container.querySelector('#ai-kitchen-btn')?.addEventListener('click', () => {
    ...
    sessionStorage.setItem('cocher_v5_1_spatial_preset', 'stations'); // stations preset works for kitchen layout
...
function renderPlanContent(el) {
  ...
  el.querySelector('#critical-friend-btn')?.addEventListener('click', async () => {
    ...
    if (!panel || !btn) return;
    let espoused = '';
    try { espoused = localStorage.getItem('cocher_v5_1_espoused_ideology') || ''; } catch { /* ignore */ }
  ...
  backdrop.querySelector('[data-action="generate"]').addEventListener('click', async () => {
    ...
    try {
      ...
      statusEl.querySelector('#arrange-classroom-btn')?.addEventListener('click', () => {
        ...
        sessionStorage.setItem('cocher_v5_1_spatial_preset', presetMap[activityType] || 'pods');
        sessionStorage.setItem('cocher_v5_1_spatial_student_count', cls.students.length.toString());
        sessionStorage.setItem('cocher_v5_1_spatial_activity', activityType);
        navigate('/spatial');
      });
    } catch (err) {
