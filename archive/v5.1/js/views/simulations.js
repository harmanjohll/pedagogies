...
function getCustomSims() {
  try {
    const raw = localStorage.getItem('cocher_v5_1_custom_sims');
    return raw ? JSON.parse(raw) : [];
  } catch { return []; }
}

function saveCustomSims(sims) {
  localStorage.setItem('cocher_v5_1_custom_sims', JSON.stringify(sims));
}
...
const SIM_DISPLAY_LIMIT_KEY = 'cocher_v5_1_sim_display_limit';
const SIM_FILTER_KEY = 'cocher_v5_1_sim_filter';
...
export function render(container) {
  ...
  const openSimId = sessionStorage.getItem('cocher_v5_1_open_sim_id');
  if (openSimId) {
    sessionStorage.removeItem('cocher_v5_1_open_sim_id');
    (async () => {
