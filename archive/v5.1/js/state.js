...
const STORAGE_KEY = 'cocher_v5_1_app_data';
...
const DEFAULT_STATE = {
  apiKey: localStorage.getItem('cocher_v5_1_api_key') || '',
  model: localStorage.getItem('cocher_v5_1_model') || 'gemini-2.5-flash',
  darkMode: localStorage.getItem('cocher_v5_1_dark_mode') === 'true',
  palette: localStorage.getItem('cocher_v5_1_palette') || '',
...
if (!_state.apiKey && localStorage.getItem('cocher_v5_1_api_key')) {
  _state.apiKey = localStorage.getItem('cocher_v5_1_api_key');
}
...
(function migrateLegacyLibraries() {
  ...
  if (!_state.stimulusLibrary?.length) {
    try {
      const raw = localStorage.getItem('cocher_v5_1_stimulus_library');
      if (raw) { _state.stimulusLibrary = JSON.parse(raw); dirty = true; }
    } catch {}
  }
  if (!_state.sourceLibrary?.length) {
    try {
      const raw = localStorage.getItem('cocher_v5_1_source_library');
...
export const Store = {
  ...
  _persist() {
    ...
    localStorage.setItem('cocher_v5_1_api_key', _state.apiKey || '');
    localStorage.setItem('cocher_v5_1_model', _state.model || 'gemini-2.5-flash');
    localStorage.setItem('cocher_v5_1_dark_mode', _state.darkMode ? 'true' : 'false');
    localStorage.setItem('cocher_v5_1_palette', _state.palette || '');
  },
  ...
  addStimulusMaterial(data) {
    ...
    localStorage.setItem('cocher_v5_1_stimulus_library', JSON.stringify(_state.stimulusLibrary));
  ...
  addSourceAnalysis(data) {
    ...
    localStorage.setItem('cocher_v5_1_source_library', JSON.stringify(_state.sourceLibrary));
  ...
  exportData() {
    ...
    const stimLib = _state.stimulusLibrary?.length ? _state.stimulusLibrary
      : (() => { try { return JSON.parse(localStorage.getItem('cocher_v5_1_stimulus_library') || '[]'); } catch { return []; } })();
    const srcLib = _state.sourceLibrary?.length ? _state.sourceLibrary
      : (() => { try { return JSON.parse(localStorage.getItem('cocher_v5_1_source_library') || '[]'); } catch { return []; } })();
  ...
  importData(jsonStr) {
    try {
      ...
      if (Array.isArray(data.stimulusLibrary)) localStorage.setItem('cocher_v5_1_stimulus_library', JSON.stringify(data.stimulusLibrary));
      if (Array.isArray(data.sourceLibrary)) localStorage.setItem('cocher_v5_1_source_library', JSON.stringify(data.sourceLibrary));
      this._persist();
      this._notify();
      return true;
    } catch {
  ...
  clearAllData() {
    ...
    localStorage.removeItem('cocher_v5_1_stimulus_library');
    localStorage.removeItem('cocher_v5_1_source_library');
