...
function getQuickActions() {
  return [
    ...
    { type: 'action', icon: '&#9881;', color: '#64748b', title: 'Manage Enactment tools', subtitle: 'Turn generator tools on or off',
      searchText: 'manage enactment tools marketplace eee settings action', action: () => { sessionStorage.setItem('cocher_v5_1_settings_tab', 'planner'); navigate('/settings'); } },
...
function getAllSearchableItems() {
  ...
  try {
    const sims = JSON.parse(localStorage.getItem('cocher_v5_1_custom_sims') || '[]');
    sims.forEach(s => {
      items.push({
  ...
  const legacyStim = stimLib.length ? stimLib : (() => { try { return JSON.parse(localStorage.getItem('cocher_v5_1_stimulus_library') || '[]'); } catch { return []; } })();
  ...
  const legacySrc = srcLib.length ? srcLib : (() => { try { return JSON.parse(localStorage.getItem('cocher_v5_1_source_library') || '[]'); } catch { return []; } })();
  legacySrc.forEach(s => {
    items.push({
