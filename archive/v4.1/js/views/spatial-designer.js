...
- "insights": array of 3 objects, each with "title" (string), "affordance" (string describing how the layout supports learning), and "moves" (array of 2 suggested teaching moves)`;
  ...
  let scenes = JSON.parse(localStorage.getItem('cocher_v4_1_scenes') || '[]');
  let activeSceneIdx = scenes.length > 0 ? 0 : -1;

  function persistScenes() {
    try { localStorage.setItem('cocher_v4_1_scenes', JSON.stringify(scenes)); } catch {}
  }
...
- "tip": one short teaching tip for this phase`;
  ...
  container.querySelector('#use-in-lesson-btn')?.addEventListener('click', () => {
    ...
    sessionStorage.setItem('cocher_v4_1_link_spatial_layout', saved.id);
  ...
  const spatialPresetFromPlanner = sessionStorage.getItem('cocher_v4_1_spatial_preset');
  const spatialStudentCount = sessionStorage.getItem('cocher_v4_1_spatial_student_count');
  const spatialActivity = sessionStorage.getItem('cocher_v4_1_spatial_activity');
  if (spatialPresetFromPlanner) {
    sessionStorage.removeItem('cocher_v4_1_spatial_preset');
    sessionStorage.removeItem('cocher_v4_1_spatial_student_count');
    sessionStorage.removeItem('cocher_v4_1_spatial_activity');
