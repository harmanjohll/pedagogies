...
const DASH_PREFS_KEY = 'cocher_v4_1_dashboard_prefs';
...
export function render(container) {
      ...
      <div class="page-container" style="max-width: 960px;">
          ...
          <div style="margin-bottom: var(--sp-5);">
            ...
            <div style="display: grid; grid-template-columns: 1fr 1fr; gap: 8px; max-width: 460px;">
              ${[
                ...
                const active = (localStorage.getItem('cocher_v4_1_theme_classic') === '1') === (t.id === 'classic');
                return `
        ...
        <div class="card" style="margin-bottom: var(--sp-6);">
          ...
          <div style="display: grid; grid-template-columns: repeat(auto-fit, minmax(190px, 1fr)); gap: 8px;">
            ${(() => {
              let current = '';
              try { current = localStorage.getItem('cocher_v4_1_espoused_ideology') || ''; } catch { /* ignore */ }
              return [
  ...
  const requestedTab = sessionStorage.getItem('cocher_v4_1_settings_tab');
  if (requestedTab) {
    sessionStorage.removeItem('cocher_v4_1_settings_tab');
  ...
  container.querySelectorAll('.design-theme-choice').forEach(btn => {
    btn.addEventListener('click', () => {
      ...
      try {
        if (classic) localStorage.setItem('cocher_v4_1_theme_classic', '1');
        else localStorage.removeItem('cocher_v4_1_theme_classic');
      } catch { /* ignore */ }
  ...
  container.querySelector('#revert-look-btn')?.addEventListener('click', () => {
    try { localStorage.setItem('cocher_v4_1_theme_classic', '1'); } catch { /* ignore */ }
  ...
  container.querySelectorAll('.ideology-choice').forEach(btn => {
    btn.addEventListener('click', () => {
      try { localStorage.setItem('cocher_v4_1_espoused_ideology', btn.dataset.ideology); } catch { /* ignore */ }
  ...
  container.querySelector('#clear-samples-btn').addEventListener('click', async () => {
    ...
    if (ok) {
      ...
      ['cocher_v4_1_seeded', 'cocher_v4_1_pd_seeded', 'cocher_v4_1_lessons_seeded',
       'cocher_v4_1_assessment_seeded', 'cocher_v4_1_exemplars_seeded_v1',
       'cocher_v4_1_portal_demos_seeded_v1'].forEach(k => localStorage.setItem(k, '1'));
      // Portal demos that live outside the main Store snapshot
      localStorage.removeItem('cocher_v4_1_cce_discussions');
