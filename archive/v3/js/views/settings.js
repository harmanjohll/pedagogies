...
const DASH_PREFS_KEY = 'cocher_v3_dashboard_prefs';
...
export function render(container) {
  ...
  const requestedTab = sessionStorage.getItem('cocher_v3_settings_tab');
  if (requestedTab) {
    sessionStorage.removeItem('cocher_v3_settings_tab');
  ...
  container.querySelector('#clear-samples-btn').addEventListener('click', async () => {
    ...
    if (ok) {
      ...
      ['cocher_v3_seeded', 'cocher_v3_pd_seeded', 'cocher_v3_lessons_seeded',
       'cocher_v3_assessment_seeded', 'cocher_v3_exemplars_seeded_v1',
       'cocher_v3_portal_demos_seeded_v1'].forEach(k => localStorage.setItem(k, '1'));
      // Portal demos that live outside the main Store snapshot
      localStorage.removeItem('cocher_v3_cce_discussions');
