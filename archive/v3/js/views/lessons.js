...
export function renderDetail(container, { id }) {
  ...
  container.querySelector('#lifecycle-cta')?.addEventListener('click', (e) => {
    ...
    if (action === 'plan') {
      ...
      sessionStorage.setItem('cocher_v3_rehearse_lesson_id', id);
  ...
  container.querySelector('#use-reflection-btn')?.addEventListener('click', () => {
    ...
    sessionStorage.setItem('cocher_v3_reflection_insights', JSON.stringify({
    ...
    if (lesson.classId) {
      ...
      if (cls) {
        sessionStorage.setItem('cocher_v3_plan_class_id', cls.id);
        sessionStorage.setItem('cocher_v3_plan_class_name', cls.name);
        sessionStorage.setItem('cocher_v3_plan_class_subject', cls.subject || '');
        sessionStorage.setItem('cocher_v3_plan_class_level', cls.level || '');
      }
    }
