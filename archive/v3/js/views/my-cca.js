...
function getCCAReminders() {
  try { return JSON.parse(localStorage.getItem('cocher_v3_cca_reminders') || '[]'); } catch { return []; }
}
function saveCCAReminders(list) {
  localStorage.setItem('cocher_v3_cca_reminders', JSON.stringify(list));
}
...
function getCCAList() {
  try { return JSON.parse(localStorage.getItem('cocher_v3_cca_list') || '[]'); } catch { return []; }
}
function saveCCAList(list) {
  localStorage.setItem('cocher_v3_cca_list', JSON.stringify(list));
}
function getLEAPSProgress() {
  try { return JSON.parse(localStorage.getItem('cocher_v3_leaps_progress') || '{}'); } catch { return {}; }
}
function saveLEAPSProgress(p) {
  localStorage.setItem('cocher_v3_leaps_progress', JSON.stringify(p));
}
