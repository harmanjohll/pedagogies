...
function getCustomSims() {
  try {
    const raw = localStorage.getItem('cocher_v3_custom_sims');
    return raw ? JSON.parse(raw) : [];
  } catch { return []; }
}

function saveCustomSims(sims) {
  localStorage.setItem('cocher_v3_custom_sims', JSON.stringify(sims));
}
...
const SIM_DISPLAY_LIMIT_KEY = 'cocher_v3_sim_display_limit';
const SIM_FILTER_KEY = 'cocher_v3_sim_filter';
