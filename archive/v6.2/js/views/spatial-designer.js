      ...
      if (slot?.free) {
        ...
        if (clsMatch) {
          ...
          container.querySelector('#tt-plan-now')?.addEventListener('click', () => {
            sessionStorage.setItem('cocher_v6_2_plan_class_id', clsMatch.id);
            sessionStorage.setItem('cocher_v6_2_plan_class_name', clsMatch.name || '');
            sessionStorage.setItem('cocher_v6_2_plan_class_subject', clsMatch.subject || '');
            sessionStorage.setItem('cocher_v6_2_plan_class_level', clsMatch.level || '');
...
- "insights": array of 3 objects, each with "title" (string), "affordance" (string describing how the layout supports learning), and "moves" (array of 2 suggested teaching moves)`;
  ...
  // and restore when a saved layout is loaded. 'cocher_v6_2_scenes' remains only a
  ...
  let scenes = [];
  try { scenes = JSON.parse(localStorage.getItem('cocher_v6_2_scenes') || '[]') || []; } catch { scenes = []; }
  ...
  function persistScenes() {
    try { localStorage.setItem('cocher_v6_2_scenes', JSON.stringify(scenes)); } catch {}
  }
...
- "tip": one short teaching tip for this phase`;
  ...
  container.querySelector('#use-in-lesson-btn')?.addEventListener('click', () => {
    ...
    sessionStorage.setItem('cocher_v6_2_link_spatial_layout', saved.id);
  ...
  const spatialPresetFromPlanner = sessionStorage.getItem('cocher_v6_2_spatial_preset');
  const spatialStudentCount = sessionStorage.getItem('cocher_v6_2_spatial_student_count');
  const spatialActivity = sessionStorage.getItem('cocher_v6_2_spatial_activity');
  if (spatialPresetFromPlanner) {
    sessionStorage.removeItem('cocher_v6_2_spatial_preset');
    sessionStorage.removeItem('cocher_v6_2_spatial_student_count');
    sessionStorage.removeItem('cocher_v6_2_spatial_activity');
  ...
  // Consumes sessionStorage 'cocher_v6_2_place_groups' written by the planner:
  ...
  (function initPlaceGroupsMode() {
    const raw = sessionStorage.getItem('cocher_v6_2_place_groups');
    if (!raw) return;
    let payload = null;
    ...
    if (!lesson || !segment || groups.length === 0) {
      ...
      sessionStorage.removeItem('cocher_v6_2_place_groups');
