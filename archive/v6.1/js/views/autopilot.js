...
export function render(container) {
  ...
  function consumePendingRefinement() {
    let raw = null;
    try { raw = sessionStorage.getItem('cocher_v6_1_plus_refinement'); } catch { raw = null; }
    if (!raw) return;
    try { sessionStorage.removeItem('cocher_v6_1_plus_refinement'); } catch { /* ignore */ }
    let data = null;
      ...
      btn.addEventListener('click', () => {
        ...
        if (action === 'planner') {
          ...
          if (expandedDive?.text) {
            try {
              sessionStorage.setItem('cocher_v6_1_plus_refinement', JSON.stringify({
