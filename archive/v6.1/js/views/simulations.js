...
function getCustomSims() {
  try {
    const raw = localStorage.getItem('cocher_v6_1_custom_sims');
    return raw ? JSON.parse(raw) : [];
  } catch { return []; }
}

function saveCustomSims(sims) {
  localStorage.setItem('cocher_v6_1_custom_sims', JSON.stringify(sims));
}
 ...
 * 'cocher_v6_1_sim_builder_prefill' ({subject?, level?, topic?, simType?,
...
const SIM_DISPLAY_LIMIT_KEY = 'cocher_v6_1_sim_display_limit';
const SIM_FILTER_KEY = 'cocher_v6_1_sim_filter';
...
export function render(container) {
  ...
  try {
    const rawPrefill = sessionStorage.getItem('cocher_v6_1_sim_builder_prefill');
    if (rawPrefill) {
      sessionStorage.removeItem('cocher_v6_1_sim_builder_prefill');
  ...
  const openSimId = sessionStorage.getItem('cocher_v6_1_open_sim_id');
  if (openSimId) {
    sessionStorage.removeItem('cocher_v6_1_open_sim_id');
    (async () => {
