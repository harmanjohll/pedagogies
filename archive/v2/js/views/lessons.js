...
export function renderDetail(container, { id }) {
  ...
  container.querySelector('#use-reflection-btn')?.addEventListener('click', () => {
    ...
    sessionStorage.setItem('cocher_v2_reflection_insights', JSON.stringify({
    ...
    if (lesson.classId) {
      ...
      if (cls) {
        sessionStorage.setItem('cocher_v2_plan_class_id', cls.id);
        sessionStorage.setItem('cocher_v2_plan_class_name', cls.name);
        sessionStorage.setItem('cocher_v2_plan_class_subject', cls.subject || '');
        sessionStorage.setItem('cocher_v2_plan_class_level', cls.level || '');
      }
    }
