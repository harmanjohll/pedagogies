...
function getAllSearchableItems() {
  ...
  const legacyStim = stimLib.length ? stimLib : (() => { try { return JSON.parse(localStorage.getItem('cocher_v2_stimulus_library') || '[]'); } catch { return []; } })();
  ...
  const legacySrc = srcLib.length ? srcLib : (() => { try { return JSON.parse(localStorage.getItem('cocher_v2_source_library') || '[]'); } catch { return []; } })();
  legacySrc.forEach(s => {
    items.push({
