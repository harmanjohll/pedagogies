...
export function renderDetail(container, { id }) {
  ...
  container.querySelector('#lifecycle-studio')?.addEventListener('click', () => {
    sessionStorage.setItem('cocher_v6_rehearse_lesson_id', id);
  ...
  container.querySelectorAll('.linked-resource-chip').forEach(chip => {
    chip.addEventListener('click', () => {
      ...
      if (type === 'simulation') {
        if (chip.dataset.resId) sessionStorage.setItem('cocher_v6_open_sim_id', chip.dataset.resId);
  ...
  container.querySelector('#use-reflection-btn')?.addEventListener('click', () => {
    ...
    sessionStorage.setItem('cocher_v6_reflection_insights', JSON.stringify({
    ...
    if (lesson.classId) {
      ...
      if (cls) {
        sessionStorage.setItem('cocher_v6_plan_class_id', cls.id);
        sessionStorage.setItem('cocher_v6_plan_class_name', cls.name);
        sessionStorage.setItem('cocher_v6_plan_class_subject', cls.subject || '');
        sessionStorage.setItem('cocher_v6_plan_class_level', cls.level || '');
      }
    }
