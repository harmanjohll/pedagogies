...
function getCollapsedSections() {
  try { return JSON.parse(localStorage.getItem('cocher_v6_sidebar_collapsed') || '[]'); } catch { return []; }
}
function setCollapsedSections(arr) {
  localStorage.setItem('cocher_v6_sidebar_collapsed', JSON.stringify(arr));
}
  ...
  container.querySelectorAll('.sidebar-item[data-route]').forEach(btn => {
    btn.addEventListener('click', () => {
      ...
      if (btn.dataset.route === '__manage_tools__') {
        sessionStorage.setItem('cocher_v6_settings_tab', 'planner');
        navigate('/settings');
        return;
      }
