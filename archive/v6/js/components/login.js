...
export function getCurrentUser() {
  try {
    const raw = localStorage.getItem('cocher_v6_current_user');
...
export function setCurrentUser(user) {
  localStorage.setItem('cocher_v6_current_user', JSON.stringify(user));
}

export function clearCurrentUser() {
  localStorage.removeItem('cocher_v6_current_user');
  // Clear API key so next user must enter their own
  Store.set('apiKey', '');
  localStorage.removeItem('cocher_v6_api_key');
}
