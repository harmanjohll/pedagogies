...
function getQuickActions() {
  return [
    ...
    { type: 'action', icon: '&#9881;', color: '#64748b', title: 'Manage Enactment tools', subtitle: 'Turn generator tools on or off',
      searchText: 'manage enactment tools marketplace eee settings action', action: () => { sessionStorage.setItem('cocher2_settings_tab', 'planner'); navigate('/settings'); } },
...
function getAllSearchableItems() {
  ...
  try {
    const arts = JSON.parse(localStorage.getItem('cocher2_library') || '[]');
    ...
    arts.forEach(a => {
      ...
      items.push({
        ...
        action: () => {
          try { sessionStorage.setItem('cocher2_open_artifact', a.id); } catch { /* ignore */ }
          navigate(route);
        }
      });
    });
  ...
  try {
    const decks = JSON.parse(localStorage.getItem('cocher2_decks') || '[]');
    decks.forEach(d => {
      items.push({
    ...
    const clips = JSON.parse(localStorage.getItem('cocher2_audio_clips') || '[]');
    clips.forEach(c => {
      items.push({
  ...
  try {
    const sims = JSON.parse(localStorage.getItem('cocher2_custom_sims') || '[]');
    sims.forEach(s => {
      items.push({
  ...
  const legacyStim = stimLib.length ? stimLib : (() => { try { return JSON.parse(localStorage.getItem('cocher2_stimulus_library') || '[]'); } catch { return []; } })();
  ...
  const legacySrc = srcLib.length ? srcLib : (() => { try { return JSON.parse(localStorage.getItem('cocher2_source_library') || '[]'); } catch { return []; } })();
  legacySrc.forEach(s => {
    items.push({
