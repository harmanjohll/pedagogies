...
function getCCAReminders() {
  try { return JSON.parse(localStorage.getItem('cocher2_cca_reminders') || '[]'); } catch { return []; }
}
function saveCCAReminders(list) {
  localStorage.setItem('cocher2_cca_reminders', JSON.stringify(list));
}
...
function saveLEAPSProgress(p) {
  localStorage.setItem('cocher2_leaps_progress', JSON.stringify(p));
}
