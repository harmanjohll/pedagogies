...
const PLANNER_REF_KEY = 'cocher2_planner_reference_ids';
...
const PLANNER_ART_KEY = 'cocher2_planner_artifact_ids';
 ...
 * render() stashes cocher2_pending_spatial_layout when a layout arrives from the
...
function consumePendingSpatialLayout() {
  const pendingId = sessionStorage.getItem('cocher2_pending_spatial_layout');
  if (!pendingId || !currentLessonId) return;
  sessionStorage.removeItem('cocher2_pending_spatial_layout');
...
const LP_PREFS_KEY = 'cocher2_lp_prefs';
function getLPPrefs() {
...
const EEE_KEY = 'cocher2_eee_selections';
const EEE_SIDEBAR_KEY = 'cocher2_eee_sidebar';
...
const CUSTOM_LINKS_KEY = 'cocher2_custom_links';
...
export function renderNew(container) {
  ...
  if (currentLessonId && sessionStorage.getItem('cocher2_link_spatial_layout')) {
...
export function render(container) {
  ...
  const planClassId = sessionStorage.getItem('cocher2_plan_class_id');
  if (planClassId && !currentLessonId && chatMessages.length === 0) {
    planClassContext = {
      id: planClassId,
      name: sessionStorage.getItem('cocher2_plan_class_name') || '',
      subject: sessionStorage.getItem('cocher2_plan_class_subject') || '',
      level: sessionStorage.getItem('cocher2_plan_class_level') || ''
    };
    sessionStorage.removeItem('cocher2_plan_class_id');
    sessionStorage.removeItem('cocher2_plan_class_name');
    sessionStorage.removeItem('cocher2_plan_class_subject');
    sessionStorage.removeItem('cocher2_plan_class_level');
  }
  ...
  // semantics as cocher2_plan_class_* above); an unsaved draft keeps priority
  // and the prefill applies on the next fresh conversation instead.
  const plannerPrefillRaw = sessionStorage.getItem('cocher2_planner_prefill');
  if (plannerPrefillRaw && !currentLessonId && chatMessages.length === 0) {
    sessionStorage.removeItem('cocher2_planner_prefill');
    try {
  ...
  const reflectionInsightsRaw = sessionStorage.getItem('cocher2_reflection_insights');
  ...
  if (reflectionInsightsRaw && !currentLessonId && chatMessages.length === 0) {
    try {
      ...
      sessionStorage.removeItem('cocher2_reflection_insights');
    } catch { sessionStorage.removeItem('cocher2_reflection_insights'); }
  }
  // NOTE: when a draft is in progress the key is deliberately KEPT (like
  // cocher2_planner_prefill) so "Plan next lesson with insights" isn't silently
  // discarded — it applies on the next fresh conversation instead.

  // Pick up spatial layout link from Spatial Designer
  const incomingSpatialId = sessionStorage.getItem('cocher2_link_spatial_layout');
  if (incomingSpatialId) {
    sessionStorage.removeItem('cocher2_link_spatial_layout');
    ...
    if (!currentLessonId) {
      sessionStorage.setItem('cocher2_pending_spatial_layout', incomingSpatialId);
    }
  }
  ...
  // lesson via the lessonId in 'cocher2_sim_builder_prefill'.
  container.querySelector('#build-sim-btn')?.addEventListener('click', () => {
    ...
    try {
      sessionStorage.setItem('cocher2_sim_builder_prefill', JSON.stringify(prefill));
    } catch {
...
Maximum 3-5 recommendations. If nothing genuinely fits, say so — don't pad with irrelevant items.`,
  ...
  container.querySelector('#ai-kitchen-btn')?.addEventListener('click', () => {
    ...
    sessionStorage.setItem('cocher2_spatial_preset', 'stations'); // stations preset works for kitchen layout
...
function renderPlanContent(el) {
  ...
  el.querySelector('#critical-friend-btn')?.addEventListener('click', async () => {
    ...
    if (!panel || !btn) return;
    let espoused = '';
    try { espoused = localStorage.getItem('cocher2_espoused_ideology') || ''; } catch { /* ignore */ }
...
function showRunOfShowEditor(container, runOfShow) {
  ...
  const handlePlaceInRoom = (idx) => {
    ...
    try {
      sessionStorage.setItem('cocher2_place_groups', JSON.stringify({
        lessonId: currentLessonId,
        segmentId: seg.id,
        groups
      }));
 ...
 * Designer handoff ('cocher2_place_groups'). Sources, in order: the segment's
  ...
  backdrop.querySelector('[data-action="generate"]').addEventListener('click', async () => {
    ...
    try {
      ...
      statusEl.querySelector('#arrange-classroom-btn')?.addEventListener('click', () => {
        ...
        sessionStorage.setItem('cocher2_spatial_preset', presetMap[activityType] || 'pods');
        sessionStorage.setItem('cocher2_spatial_student_count', cls.students.length.toString());
        sessionStorage.setItem('cocher2_spatial_activity', activityType);
        navigate('/spatial');
      });
    } catch (err) {
...
function renderSpatialContextBar(container) {
  ...
  barEl.querySelector('#spatial-context-open')?.addEventListener('click', () => {
    ...
    try {
      sessionStorage.setItem('cocher2_open_layout', JSON.stringify({
        layoutId: layout.id,
