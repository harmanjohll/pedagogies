  ...
  container.querySelector('#import-lesson-btn')?.addEventListener('click', () => {
    import('../components/modals.js').then(({ openModal }) => {
      const { backdrop, close } = openModal({
        ...
        body: `
          ...
          <div class="input-group">
            ...
            <textarea class="input" id="import-json-paste" rows="6" placeholder='{"_cocher2_lesson": true, "title": "...", ...}'></textarea>
          </div>
        `,
        footer: `
      ...
      function processImport(data) {
        if (!data._cocher2_lesson || !data.title) throw new Error('Invalid');
...
export function renderDetail(container, { id }) {
  ...
  container.querySelector('#lifecycle-studio')?.addEventListener('click', () => {
    sessionStorage.setItem('cocher2_rehearse_lesson_id', id);
  ...
  container.querySelectorAll('.linked-resource-chip').forEach(chip => {
    chip.addEventListener('click', () => {
      ...
      if (type === 'simulation') {
        if (chip.dataset.resId) sessionStorage.setItem('cocher2_open_sim_id', chip.dataset.resId);
  ...
  container.querySelector('#use-reflection-btn')?.addEventListener('click', () => {
    ...
    sessionStorage.setItem('cocher2_reflection_insights', JSON.stringify({
    ...
    if (lesson.classId) {
      ...
      if (cls) {
        sessionStorage.setItem('cocher2_plan_class_id', cls.id);
        sessionStorage.setItem('cocher2_plan_class_name', cls.name);
        sessionStorage.setItem('cocher2_plan_class_subject', cls.subject || '');
        sessionStorage.setItem('cocher2_plan_class_level', cls.level || '');
      }
    }
