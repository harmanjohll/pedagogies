...
function getCustomSims() {
  try {
    const raw = localStorage.getItem('cocher2_custom_sims');
    return raw ? JSON.parse(raw) : [];
  } catch { return []; }
}

function saveCustomSims(sims) {
  localStorage.setItem('cocher2_custom_sims', JSON.stringify(sims));
}
 ...
 * 'cocher2_sim_builder_prefill' ({subject?, level?, topic?, simType?,
...
const SIM_DISPLAY_LIMIT_KEY = 'cocher2_sim_display_limit';
const SIM_FILTER_KEY = 'cocher2_sim_filter';
...
export function render(container) {
  ...
  try {
    const rawPrefill = sessionStorage.getItem('cocher2_sim_builder_prefill');
    if (rawPrefill) {
      sessionStorage.removeItem('cocher2_sim_builder_prefill');
  ...
  const openSimId = sessionStorage.getItem('cocher2_open_sim_id');
  if (openSimId) {
    sessionStorage.removeItem('cocher2_open_sim_id');
    (async () => {
