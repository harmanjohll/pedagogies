    ...
    btn.addEventListener('click', (e) => {
      ...
      if (action === 'open' && lessonId) {
        ...
        if (clsId) {
          ...
          if (cls) {
            // Same handoff keys the lesson planner reads (cocher2_plan_class_*).
...
const MILESTONES_SEEN_KEY = 'cocher2_milestones_seen';
...
export function render(container) {
  ...
  container.querySelector('#up-next-cta')?.addEventListener('click', (e) => {
    ...
    if (btn.dataset.action === 'plan') {
      ...
      sessionStorage.setItem('cocher2_rehearse_lesson_id', lessonId);
