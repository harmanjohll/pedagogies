...
export function render(container) {
  ...
  /* Consume the cocher2_plus_refinement handoff (written by the deep-dive
  ...
  function consumePendingRefinement() {
    let raw = null;
    try { raw = sessionStorage.getItem('cocher2_plus_refinement'); } catch { raw = null; }
    if (!raw) return;
    try { sessionStorage.removeItem('cocher2_plus_refinement'); } catch { /* ignore */ }
    let data = null;
      ...
      btn.addEventListener('click', () => {
        ...
        if (action === 'planner') {
          ...
          if (expandedDive?.text) {
            try {
              sessionStorage.setItem('cocher2_plus_refinement', JSON.stringify({
